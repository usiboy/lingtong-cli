...
7. 配置数据视图 → 自动调用 lingtong-scene-data-view-create
```

## 推荐方案

对于需要完整配置的场景，使用 **`lingtong-scene-create` skill**：

```
lingtong-scene-create
    ↓ 自动调用
lingtong-scene-data-view-create
    ↓ 自动调用
lingtong-scene-data-preprocess-create
    ↓ 自动调用
lingtong-scene-field-mapping-auto-analyse
    ↓ 自动调用
lingtong-scene-field-mapping-create
```
...
| 创建可运行的完整场景 | `lingtong-scene-create` skill |
| 批量创建场景 | `lingtong-scene-create` skill |
| 创建带触发器的场景 | `lingtong-scene-create` skill |
